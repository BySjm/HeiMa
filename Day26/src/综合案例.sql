...
SELECT e.id,e.ename,e.salary,j.jname,j.description,d.dname,d.loc,s.grade FROM emp e
	INNER JOIN job j ON e.job_id = j.id 
	INNER JOIN dept d ON e.dept_id = d.id 
...
SELECT d.id,d.dname,d.loc,COUNT(*) FROM emp e INNER JOIN dept d ON e.dept_id = d.id GROUP BY d.id;
...
SELECT d.id,d.dname,d.loc,e.total FROM dept d
	LEFT JOIN (SELECT dept_id, COUNT(*) AS total FROM emp GROUP BY dept_id) e
	ON d.id = e.dept_id;